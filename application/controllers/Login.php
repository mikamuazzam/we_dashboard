...
class Login extends CI_Controller {
	...
	public function index()
	{
		...
		else  if ($this->session->userdata('user_login_access') == 1 and $this->session->userdata('dept')==9) 
    ...
    function validate_login($email = '', $password = '') {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session->set_userdata('user_log_id', $row->id);
			...
			$this->session->set_userdata('dept', $row->dep_id);
            $this->session->set_userdata('user_type', $row->em_role);
			$this->session->set_userdata('partner_id', $row->em_partnerid);
            return 'success';
        }
	}
    /*Logout method*/
